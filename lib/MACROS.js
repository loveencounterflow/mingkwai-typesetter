(function() {
  ...
  this.action_and_region_patterns = [/(^|[^\\])<<\(([.:]?)((?:\\>|[^>.:]|>(?!>))*)>>((?:\\<|[^<]|<(?!<))*)<<((?:\2\3)?)\)>>/g];
  ...
  this.escape = (function(_this) {
    return function(S, text) {
      ...
      R = _this.escape.action_and_region_macros(S, R);
  ...
  this.escape.action_and_region_macros = (function(_this) {
    return function(S, text) {
      var R, i, len, pattern, ref;
      R = text;
      ref = _this.action_and_region_patterns;
      for (i = 0, len = ref.length; i < len; i++) {
        pattern = ref[i];
        R = R.replace(pattern, function(_, previous_chr, markup, identifier, content, stopper) {
          var id, language, mode, starter_id, stopper_id;
          debug('©ΛΨΒΓΘ', [previous_chr, markup, identifier, content]);
          switch (markup) {
            case '':
              starter_id = _this._register_content(S, 'region', '(', identifier);
              stopper_id = _this._register_content(S, 'region', ')', identifier);
  ...
  this.$expand_html_comments = (function(_this) {
    return function(S) {
      ...
      return $(function(event, send) {
        var content, entry, i, id, is_plain, len, meta, name, ref, results, stretch, text, type;
  ...
  this.$expand_action_macros = (function(_this) {
    return function(S) {
      ...
      /* TAINT code duplication */
      return $(function(event, send) {
        var content, entry, i, id, is_plain, language, len, meta, mode, name, ref, ref1, results, stretch, text, type;
        if (MKTS.select(event, '.', 'text')) {
          is_plain = false;
          type = event[0], name = event[1], text = event[2], meta = event[3];
          ...
          for (i = 0, len = ref.length; i < len; i++) {
            ...
            if (!is_plain) {
              id = parseInt(stretch, 10);
              entry = _this._retrieve_entry(S, id);
              ref1 = entry['markup'], mode = ref1[0], language = ref1[1];
              content = entry['raw'];
              results.push(send([
  ...
  this.$expand_region_macros = (function(_this) {
    return function(S) {
      ...
      /* TAINT code duplication */
      return $(function(event, send) {
        var entry, i, id, is_plain, len, markup, meta, name, raw, ref, results, stretch, text, type;
        if (MKTS.select(event, '.', 'text')) {
          is_plain = false;
          type = event[0], name = event[1], text = event[2], meta = event[3];
          ref = text.split(_this.region_id_pattern);
          results = [];
          for (i = 0, len = ref.length; i < len; i++) {
            stretch = ref[i];
            is_plain = !is_plain;
            if (!is_plain) {
              ...
              results.push(send([markup, raw, null, MKTS.copy(meta)]));
            } else {
              if (stretch.length !== 0) {
                results.push(send([type, name, stretch, MKTS.copy(meta)]));
              } else {
  ...
  this.$expand_command_and_value_macros = (function(_this) {
    return function(S) {
      ...
      /* TAINT code duplication */
      return $(function(event, send) {
        var entry, i, id, is_plain, len, macro_type, markup, meta, name, raw, ref, results, stretch, text, type;
        if (MKTS.select(event, '.', 'text')) {
          is_plain = false;
          type = event[0], name = event[1], text = event[2], meta = event[3];
          ref = text.split(_this.command_and_value_id_pattern);
          results = [];
          for (i = 0, len = ref.length; i < len; i++) {
            stretch = ref[i];
            is_plain = !is_plain;
            if (!is_plain) {
              ...
              results.push(send(['.', macro_type, raw, MKTS.copy(meta)]));
            } else {
              if (stretch.length !== 0) {
                results.push(send([type, name, stretch, MKTS.copy(meta)]));
              } else {
