(function() {
  ...
  this.$expand = function(S) {
    ...
    readstream.pipe(this.$expand_command_and_value_macros(S)).pipe(this.$expand_region_macros(S)).pipe(this.$expand_action_macros(S)).pipe(this.$expand_raw_macros(S)).pipe(this.$expand_html_comments(S)).pipe(this.$expand_escape_chrs(S)).pipe(writestream);
    settings = {
      S: S
    };
