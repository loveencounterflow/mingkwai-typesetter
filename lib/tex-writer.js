...
(function() {
  var $, $async, ASYNC, CACHE, CND, D, HELPERS, LINEBREAKER, MACRO_ESCAPER, MACRO_INTERPRETER, MD_READER, MKTS, MKTSCRIPT_WRITER, OPTIONS, SEMVER, TEXLIVEPACKAGEINFO, TEXT, XNCHR, after, alert, badge, before, copy, debug, echo, help, hide, info, is_hidden, is_stamped, jr, log, njs_fs, njs_path, options_route, ref, rpr, select, stamp, step, suspend, unstamp, urge, warn, whisper, ƒ, Σ_formatted_warning,
    slice = [].slice;
    ...
    return function(S) {
      ...
      return $(function(event, send) {
        ...
        if (select(event, '(', ['h', 'multi-columns', 'blockquote'], true)) {
          S.is_first_par = true;
        } else if (select(event, ')', ['blockquote', 'ul'], true)) {
          S.is_first_par = true;
        } else if (select(event, '(', ['ul'], true)) {
          S.within_ul = true;
        } else if (select(event, ')', ['ul'], true)) {
          S.within_ul = false;
        }
