(function() {
  ...
  this.$document = (function(_this) {
    return function(S) {
      var before_document_command, before_flush, buffer, flush_as, send_, start_document_event;
      buffer = [];
      ...
      return $(function(event, send) {
        ...
        if (select(event, '.', 'hr')) {
          ...
          switch (chr = text[0]) {
            ...
            case ':':
              send(stamp(hide(copy(event))));
              return send(['!', 'slash', [plain_rule], copy(meta)]);
            case '=':
              send(stamp(hide(copy(event))));
            ...
            default:
              send(stamp(hide(copy(event))));
              return send(['.', 'warning', "horizontal rule with unknown markup " + (rpr(text)), copy(meta)]);
          }
        } else {
          return send(event);
        }
    ...
    return function(S) {
      ...
      return $(function(event, send) {
        if (select(event, '(', 'em')) {
          ...
          send(stamp(event));
          if (strong_count > 0) {
            return send(['tex', "{\\mktsStyleBolditalic{}"]);
          } else {
  ...
  this.MKTX.INLINE.$smallcaps = (function(_this) {
    return function(S) {
      var sc_lower_count, sc_upper_count;
      sc_upper_count = 0;
      sc_lower_count = 0;
      return $(function(event, send) {
        if (select(event, '(', 'smallcaps-upper')) {
          ...
          send(stamp(event));
          if (sc_lower_count > 0) {
            return send(['tex', "{\\mktsStyleSmallcapsall{}"]);
          } else {
            return send(['tex', "{\\mktsStyleSmallcapsupper{}"]);
          }
          ...
          if (sc_upper_count > 0) {
            ...
            return send(['tex', "{\\mktsStyleSmallcapslower{}"]);
          }
        } else if (select(event, ')', 'smallcaps-lower')) {
          sc_lower_count += -1;
          send(stamp(event));
  ...
  this.MKTX.INLINE.$em_strong_and_smallcaps = (function(_this) {
    return function(S) {
      ...
      return $(function(event, send) {
        var i, j, key, len, len1, meta, name, ref1, results, start, stop, sub_event, text, type;
        type = event[0], name = event[1], text = event[2], meta = event[3];
        if (select(event, ['(', ')'], ['code', 'code-span'])) {
          code_count += type === '(' ? +1 : -1;
          return send(event);
        } else if (select(event, ['(', ')'], 'smallcaps-upper')) {
          sc_upper_count += type === '(' ? +1 : -1;
          return send(stamp(event));
        } else if (select(event, ['(', ')'], 'smallcaps-lower')) {
          sc_lower_count += type === '(' ? +1 : -1;
          return send(stamp(event));
        } else if (select(event, ['(', ')'], 'em')) {
          em_count += type === '(' ? +1 : -1;
          return send(stamp(event));
        } else if (select(event, ['(', ')'], 'strong')) {
          strong_count += type === '(' ? +1 : -1;
          return send(stamp(event));
