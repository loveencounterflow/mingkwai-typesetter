...
(function() {
  var $, $async, ASYNC, CACHE, CND, D, HELPERS, LINEBREAKER, MACRO_ESCAPER, MACRO_INTERPRETER, MD_READER, MKTS, MKTSCRIPT_WRITER, OPTIONS, SEMVER, TEXLIVEPACKAGEINFO, TEXT, XNCHR, after, alert, badge, before, copy, debug, echo, help, hide, info, is_hidden, is_stamped, log, njs_fs, njs_path, options_route, ref, rpr, select, stamp, step, suspend, unstamp, urge, warn, whisper, ƒ, Σ_formatted_warning,
    slice = [].slice;
  ...
  this.MKTX.BLOCK.$paragraph_1 = (function(_this) {
    return function(S) {
  ...
  this.MKTX.BLOCK.$paragraph_2 = (function(_this) {
    return function(S) {
      var close_paragraph, collector, seen_text_event, within_paragraph;
      ...
      return $(function(event, send) {
        ...
        if (event[0] === '~' && event[1] === 'start-paragraph') {
          ...
          if (seen_text_event) {
            ...
            if (!select(event, '.', 'text')) {
               ...
               */
              has_indent = !S.is_first_par;
              S.is_first_par = false;
              if (has_indent) {
                send(['tex', "%% with indent\n"]);
                send(['tex', "\\hskip 0mm plus \\mktsLineheight minus 0.0mm}"]);
              } else {
                send(['tex', "%% no indent\n"]);
              }
    ...
    return function(S) {
      ...
      return $(function(event, send) {
        ...
        if (select(event, '(', 'ul')) {
          ...
          return send(['tex', "{\\mktsFontfileCwtexqheibold{}\\prPushRaise{-0.4}{-0.1}{▷}}"]);
        } else if (select(event, ')', 'li')) {
          send(stamp(event));
          return send(['tex', '\n']);
  ...
  this.MKTX.$format_warnings = (function(_this) {
    return function(S) {
    ...
    return function(S) {
      ...
      return $(function(event, send) {
        ...
        if (select(event, '.', Σ_formatted_warning)) {
          _ = event[0], _ = event[1], message_tex = event[2], meta = event[3];
    ...
    return D.$observe((function(_this) {
      return function(_event) {
        ...
        if (matches(['(', '!'], ['multi-columns', 'columns'])) {
          ...
          meta.locator = ['c' + (rpr(column_count))].concat(slice.call(stack)).join('/');
        }
        return null;
      };
    })(this));
  ...
  this.$show_text_locators = function(S) {
    return D.$observe((function(_this) {
      return function(event) {
        var meta, name, ref1, text, type;
        if (select(event, '.', 'text')) {
          type = event[0], name = event[1], text = event[2], meta = event[3];
          help(CND.grey('22311'), (CND.lime((ref1 = meta.locator) != null ? ref1 : '????????????')) + ' ' + (CND.white(rpr(text))));
        }
        return null;
      };
    })(this));
  ...
  this.create_tex_write_tee = function(S) {
    ...
    })(this)()).pipe(this.MKTX.INLINE.$mark(S)).pipe(this.MKTX.$show_unhandled_tags(S)).pipe(this.MKTX.$format_warnings(S)).pipe(this.MKTX.$show_warnings(S)).pipe(this.$filter_tex(S)).pipe(this.COLUMNS.$XXX_transform_pretex_to_tex(S)).pipe(MD_READER.$show_illegal_chrs(S)).pipe(writestream);
    settings = {
      S: S
    };
