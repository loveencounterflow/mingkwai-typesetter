(function() {
  var $, $async, ASYNC, CACHE, CND, D, HELPERS, LINEBREAKER, MACRO_ESCAPER, MD_READER, MKTS, MKTSCRIPT_WRITER, OPTIONS, SEMVER, TEXLIVEPACKAGEINFO, XNCHR, alert, badge, copy, debug, echo, help, hide, info, is_hidden, is_stamped, log, njs_fs, njs_path, options_route, ref, rpr, select, stamp, step, suspend, unstamp, urge, warn, whisper, ƒ;
  ...
  this.MKTX.BLOCK.$heading = (function(_this) {
    return function(S) {
      ...
      h_idx = null;
      return $(function(event, send) {
        var level, meta, name, type, within_multi_column;
        within_multi_column = track.within('(multi-column)');
        track(event);
        if (select(event, '(', 'h')) {
          ...
          switch (level) {
            case 1:
              return send(['tex', "{\\mktsHOne{}"]);
            case 2:
              return send(['tex', "{\\mktsHTwo{}"]);
            case 3:
              return send(['tex', "{\\mktsHThree{}"]);
            default:
  ...
  this.MKTX.BLOCK.$toc = (function(_this) {
    return function(S) {
      ...
      return $(function(event, send) {
        var events, h_event, heading, i, j, len, len1, level, meta, name, text, type;
        if (select(event, '(', 'h')) {
          type = event[0], name = event[1], level = event[2], meta = event[3];
          within_heading = true;
          ...
          for (i = 0, len = headings.length; i < len; i++) {
            heading = headings[i];
            level = heading.level, events = heading.events;
