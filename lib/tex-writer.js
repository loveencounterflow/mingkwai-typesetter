(function() {
  ...
  this.MKTX.REGION.$multi_column = (function(_this) {
    return function(S) {
      ...
      remark = MD_READER._get_remark();
      return $(function(event, send) {
        var column_count, meta, name, parameters, ref1, type, within_multi_column;
        within_multi_column = track.within('(multi-column)');
        track(event);
        if (select(event, ['(', ')'], 'multi-column')) {
          send(stamp(event));
          ...
          column_count = (ref1 = parameters[0]) != null ? ref1 : 2;
          if (type === '(') {
            if (within_multi_column) {
              send(remark('drop', "`(multi-column` because already within `(multi-column)`", copy(meta)));
            } else {
              send(track(_this.MKTX.REGION._begin_multi_column(column_count)));
            }
          } else {
            if (within_multi_column) {
              send(track(_this.MKTX.REGION._end_multi_column(column_count)));
            } else {
