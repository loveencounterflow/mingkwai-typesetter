(function() {
  var $, $async, ASYNC, CACHE, CND, D, HELPERS, LINEBREAKER, MACRO_ESCAPER, MD_READER, MKTS, MKTSCRIPT_WRITER, OPTIONS, SEMVER, TEXLIVEPACKAGEINFO, XNCHR, after, alert, badge, before, copy, debug, echo, help, hide, info, is_hidden, is_stamped, log, njs_fs, njs_path, options_route, ref, rpr, select, stamp, step, suspend, unstamp, urge, warn, whisper, ƒ,
    slice = [].slice;
  ...
  this.MKTX.REGION.$multi_column = (function(_this) {
    return function(S) {
      ...
      column_count = 1;
      return $(function(event, send) {
        var meta, name, parameters, ref1, type, within_multi_column;
        within_multi_column = track.within('(multi-column)');
        track(event);
        if (select(event, ['(', ')'], 'multi-column')) {
          send(stamp(event));
          type = event[0], name = event[1], parameters = event[2], meta = event[3];
          column_count = (ref1 = parameters != null ? parameters[0] : void 0) != null ? ref1 : S.document.column_count;
          if (type === '(') {
            if (within_multi_column) {
              send(remark('drop', "`(multi-column` because already within `(multi-column)`", copy(meta)));
            } else {
              send(track(_this.MKTX.REGION._begin_multi_column(S, column_count)));
            }
          } else {
            if (within_multi_column) {
              send(track(_this.MKTX.REGION._end_multi_column(S, column_count)));
            } else {
  ...
  this.MKTX.REGION.$single_column = (function(_this) {
    return function(S) {
      ...
      column_count = 1;
      return $(function(event, send) {
        var meta, name, parameters, ref1, text, type, within_multi_column;
        within_multi_column = track.within('(multi-column)');
        track(event);
        if (select(event, ['(', ')'], 'multi-column')) {
          send(event);
  ...
  this.MKTX.REGION.$code = (function(_this) {
    return function(S) {
      ...
      return $(function(event, send) {
        var language, meta, name, text, type, within_code;
        ...
        if (select(event, '.', 'text')) {
          ...
          send(stamp(event));
          type = event[0], name = event[1], language = event[2], meta = event[3];
  ...
  this.MKTX.REGION.$keep_lines = (function(_this) {
    return function(S) {
      var last_was_empty, track;
      track = MD_READER.TRACKER.new_tracker('(keep-lines)');
      last_was_empty = false;
      return $(function(event, send) {
        var chunk, chunks, i, len, meta, name, results, text, type, within_keep_lines;
  ...
  this.MKTX.BLOCK.$heading = (function(_this) {
    return function(S) {
      var column_count, h_idx, h_nrs, restart_multicols, track;
      ...
      column_count = 1;
      return $(function(event, send) {
        var h_key, level, meta, name, parameters, ref1, type, within_multi_column;
        within_multi_column = track.within('(multi-column)');
        track(event);
        if (select(event, '(', 'multi-column')) {
          type = event[0], name = event[1], parameters = event[2], meta = event[3];
