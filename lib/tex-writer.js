(function() {
  ...
  f = function() {
    return this.remit_async_spread = function(method) {
      var $call, $show, $spread, Z, input, output;
      Z = [];
      ...
      input.pipe($show()).pipe($call()).pipe($spread()).pipe(output);
  ...
  CUSTOM.JZR.$most_frequent = (function(_this) {
    return function(S) {
      ...
      return D.TEE.from_pipeline([CUSTOM.JZR.$most_frequent.with_fncrs._$rewrite_events(S), CUSTOM.JZR.$most_frequent._$read(S), D.$show('7249374'), CUSTOM.JZR.$most_frequent.with_fncrs._$read(S), CUSTOM.JZR.$most_frequent.with_fncrs._$assemble(S), CUSTOM.JZR.$most_frequent._$assemble(S)]);
    };
  })(this);
  ...
  CUSTOM.JZR.$most_frequent.with_fncrs._$read = (function(_this) {
    return function(S) {
      ...
      HOLLERITH = require('../../hollerith');
      return D.remit_async_spread(function(event, send) {
        var _, glyph, meta, prefix, ref1, within_glyphs;
        within_glyphs = track.within('(glyphs-with-fncrs)');
        track(event);
        if ((ref1 = event[2]) === '的' || ref1 === '人') {
          debug('88723', within_glyphs, event);
        }
  ...
  CUSTOM.JZR.$most_frequent.with_fncrs._$assemble = (function(_this) {
    return function(S) {
      ...
      this_glyph = null;
      return $(function(event, send) {
        var _, glyph, meta, obj, obj_txt, prd, text, within_glyphs;
        within_glyphs = track.within('(glyphs-with-fncrs)');
        track(event);
        if (select(event, ['(', ')'], 'glyphs-with-fncrs')) {
          ...
          send(stamp(event));
          return send(['tex', "\\begin{minipage}{0.8\\linewidth}"]);
        } else if (select(event, ')', 'details')) {
          ...
          return send(['tex', "}{}"]);
        } else if (within_glyphs && select(event, '*', ['cp/fncr', 'reading/py', 'reading/hi', 'reading/ka', 'reading/gloss'])) {
          send(stamp(event));
          _ = event[0], prd = event[1], obj = event[2], meta = event[3];
          obj_txt = CND.isa_text(obj) ? obj : rpr(obj);
          text = " " + obj_txt + " ";
          return send(['.', 'text', text, copy(meta)]);
