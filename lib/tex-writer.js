(function() {
  ...
  f = function() {
    ...
    this.TEE._from_confluence = (function(_this) {
      return function(confluence, settings) {
        var input, key, output, ref1, ref2, value;
        input = (ref1 = settings['input']) != null ? ref1 : _this.create_throughstream();
        output = (ref2 = settings['output']) != null ? ref2 : _this.create_throughstream();
    ...
    return this.remit_async_spread = function(method) {
      var Z, read, write;
      Z = [];
      read = $map((function(_this) {
        return function(input_data, handler) {

          /* TAINT should add `done.end`, `done.pause` and so on */
          var send;
          send = function(output_data) {
            if (output_data != null) {
              return Z.push(output_data);
            } else {
              help('©53098', Z);
              return handler(null, 'ignore');
            }
          };
          send.done = function(output_data) {
            if (output_data != null) {
              send(output_data);
            }
            return send();
          ...
          return method(input_data, send);
        };
      })(this));
      write = $((function(_this) {
        return function(_, send) {
          var data, i, len;
          debug('©12400', _);
      ...
      return this.TEE.from_pipeline([read, write]);
    };
  };
  ...
  CUSTOM.JZR.$most_frequent = (function(_this) {
    return function(S) {
      var $most_frequent, defaults, pipeline;
      defaults = {
        n: 100
      };
      $most_frequent = function(S) {
        return D.remit_async_spread(function(event, send) {
          var meta, n, name, ref1, type;
          debug('©39546', event);
          ...
          return step(function*(resume) {
            var glyph, glyphs, i, idx, len;
            glyphs = (yield (require('../../hollerith/lib/demo')).read_sample(null, n, resume));
            glyphs = Object.keys(glyphs);
            glyphs = (function() {
              var i, len, results;
              results = [];
              for (idx = i = 0, len = glyphs.length; i < len; idx = ++i) {
                glyph = glyphs[idx];
                results.push(idx % 40 === 0 ? glyph + "\n" : glyph);
              }
              return results;
            })();
