(function() {
  var $, $async, ASYNC, CACHE, CND, D, HELPERS, LINEBREAKER, MACRO_ESCAPER, MD_READER, MKTS, MKTSCRIPT_WRITER, OPTIONS, SEMVER, TEXLIVEPACKAGEINFO, XNCHR, alert, badge, before, copy, debug, echo, help, hide, info, is_hidden, is_stamped, log, njs_fs, njs_path, options_route, ref, rpr, select, stamp, step, suspend, unstamp, urge, warn, whisper, ƒ,
    slice = [].slice;
  ...
  before('@MKTX.BLOCK.$heading', '@MKTX.BLOCK.$toc', this.MKTX.REGION.$toc = (function(_this) {
    return function(S) {
      var buffer, track;
  ...
  this.MKTX.BLOCK.$toc = (function(_this) {
    return function(S) {
      var headings, new_heading, this_heading, within_heading;
      ...
      return $(function(event, send) {
        var events, h_event, heading, i, idx, j, key, last_idx, len, len1, level, meta, name, text, type;
        type = event[0], name = event[1], text = event[2], meta = event[3];

        /* TAINT use library method to test event category */
        if ((meta != null) && (meta['toc'] !== 'omit') && select(event, '(', 'h')) {
          level = text;
          within_heading = true;
          ...
          send(event);
          return njs_fs.writeFileSync('/tmp/mkts.json', JSON.stringify(headings, null, '  '));
        } else {
          return send(event);
        }
      });
