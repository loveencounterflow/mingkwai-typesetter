(function() {
  ...
  this.MKTX.BLOCK.$heading = (function(_this) {
    return function(S) {
      var h_idx, h_nrs, level, restart_multicols, track;
      ...
      return $(function(event, send) {
        ...
        if (select(event, ['(', ')'], ['h1', 'h2', 'h3', 'h4', 'h5', 'h6'])) {
          ...
          if (type === '(') {
            ...
            return send(['!', 'mark', name, copy(meta)]);
          } else {
            send(stamp(event));
            switch (level) {
              case 1:
  ...
  this.MKTX.BLOCK.$toc = (function(_this) {
    return function(S) {
      ...
      return $(function(event, send) {
        var events, h_event, heading, i, len, level, meta, name, results, text, type;
        type = event[0], name = event[1], text = event[2], meta = event[3];
        if (select(event, '(', ['h1', 'h2', 'h3', 'h4', 'h5', 'h6'])) {
