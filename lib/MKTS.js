(function() {
  ...
  this._ESC.action_patterns = [/(^|[^\\])<<\(([.:](?:\\>|[^>]|>(?!>))*)>>((?:\\<|[^<]|<(?!<))*)<<(\2?)\)>>/g];

  this._ESC.region_patterns = [/(^|[^\\])<<\(((?:\\>|[^>]|>(?!>))*)>>((?:\\<|[^<]|<(?!<))*)<<(\2?)\)>>/g];

  this._ESC.bracketed_raw_patterns = [/(^|[^\\])<<(<)((?:\\>|[^>]|>{1,2}(?!>))*)>>>/g];

  this._ESC.command_and_value_patterns = [/(^|[^\\])<<([!$])((?:\\>|[^>]|>(?!>))*)>>/g];
  ...
  this._ESC.end_command_patterns = [/(^|^[\s\S]+[^\\])<<!end>>/];

  this._ESC.illegal_patterns = [/(^|[^\\])(<<|>>)([\s\S]{0,10})/g];
  ...
  this._ESC.match_first = (function(_this) {
  ...
  this._ESC.truncate_text_at_end_command_macro = (function(_this) {
    return function(S, text) {
      var R, match;
      if ((match = _this._ESC.match_first(_this._ESC.end_command_patterns, text)) == null) {
        return [text, 0];
      }
      R = match[1];
  ...
  this._ESC.escape_html_comments = (function(_this) {
    return function(S, text) {
      var R;
      R = text;
      R = R.replace(_this._ESC.html_comment_pattern, function(_, previous_chr, content) {
        var key;
        key = _this._ESC.register_content(S, 'comment', null, content, content.trim());
  ...
  this._ESC.escape_bracketed_raw_macros = (function(_this) {
    return function(S, text) {
      var R, i, len, pattern, ref;
      R = text;
      ref = _this._ESC.bracketed_raw_patterns;
      for (i = 0, len = ref.length; i < len; i++) {
        pattern = ref[i];
        R = R.replace(pattern, function(_, previous_chr, markup, content) {
          var id;
          id = _this._ESC.register_content(S, 'raw', markup, content);
  ...
  this._ESC.escape_action_macros = (function(_this) {
    return function(S, text) {
      var R, i, len, pattern, ref;
      R = text;
      ref = _this._ESC.action_patterns;
      for (i = 0, len = ref.length; i < len; i++) {
        pattern = ref[i];
        R = R.replace(pattern, function(_, previous_chr, starter, content, stopper) {
          ...
          id = _this._ESC.register_content(S, 'action', [mode, language], content);
          return previous_chr + "\x15" + id + "\x13";
        });
      }
      return R;
  ...
  this._ESC.escape_region_macros = (function(_this) {
    return function(S, text) {
      var R, i, len, pattern, ref;
      R = text;
      ref = _this._ESC.region_patterns;
      for (i = 0, len = ref.length; i < len; i++) {
        pattern = ref[i];
        R = R.replace(pattern, function(_, previous_chr, starter, content, stopper) {
          ...
          starter_rpr = "<<(" + starter + ">>";
          stopper_rpr = "<<" + stopper + ")>>";
          starter_id = _this._ESC.register_content(S, 'region', starter, starter_rpr);
          stopper_id = _this._ESC.register_content(S, 'region', starter, stopper_rpr);
  ...
  this._ESC.escape_command_and_value_macros = (function(_this) {
    return function(S, text) {
      var R, i, len, pattern, ref;
      R = text;
      ref = _this._ESC.command_and_value_patterns;
      for (i = 0, len = ref.length; i < len; i++) {
        pattern = ref[i];
        R = R.replace(pattern, function(_, previous_chr, markup, content) {
          ...
          parsed_content = '???';
          key = _this._ESC.register_content(S, kind, markup, content, parsed_content);
          return previous_chr + "\x15" + key + "\x13";
        });
      }
      return R;
  ...
  this._ESC.escape_macros = (function(_this) {
    return function(S, text) {
      var R, discard_count, ref;
      ref = _this._ESC.truncate_text_at_end_command_macro(S, text), R = ref[0], discard_count = ref[1];
      if (discard_count > 0) {
        whisper("detected <<!end>> macro; discarding approx. " + discard_count + " characters");
      }
      R = _this._ESC.escape_escape_chrs(S, R);
      R = _this._ESC.escape_html_comments(S, R);
      R = _this._ESC.escape_bracketed_raw_macros(S, R);
      R = _this._ESC.escape_action_macros(S, R);
      R = _this._ESC.escape_region_macros(S, R);
      R = _this._ESC.escape_command_and_value_macros(S, R);
      return R;
    };
  })(this);
  ...
  this._ESC.expand_html_comments = (function(_this) {
    return function(S, text) {
      var R, entry, i, id, idx, is_plain, len, ref, stretch;
      is_plain = false;
      ref = R = text.split(_this._ESC.html_comment_id_pattern);
    ...
    return (function(_this) {
      return function(md_source) {
        ...
        if (CND.isa_text(md_source)) {
          md_source = _this._ESC.escape_macros(S, md_source);
          environment = {};
  ...
  this.mkts_events_from_md = function(source, settings, handler) {
    var Z, arity, bare, input, md_fitting, output, ref;
    ...
    md_fitting = this.create_md_readfitting(source);
    input = md_fitting.input, output = md_fitting.output;
    Z = [];
  ...
  this.mktscript_from_md = function(md_source, settings, handler) {
    ...
    /* TAINT code duplication */
    var arity, f, input, md_fitting, output, ref, source_route;
    switch (arity = arguments.length) {
      case 2:
        handler = settings;
        settings = {};
    ...
    md_fitting = this.create_md_readfitting(md_source);
    input = md_fitting.input, output = md_fitting.output;
    f = (function(_this) {
      return function() {
        return input.resume();
    ...
    output.pipe(this.$produce_mktscript(md_fitting['S'])).pipe((function(_this) {
      return function() {
        var Z;
        Z = [];
        return $(function(event, send, end) {
  ...
  this.create_md_readfitting = function(md_source, settings) {
