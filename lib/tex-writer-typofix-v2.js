...
(function() {
  var $, CND, D, MD_READER, MKNCR, alert, badge, copy, debug, echo, help, hide, info, is_hidden, is_stamped, kw, log, rpr, select, stamp, urge, warn, whisper, Σ_glyph_description,
  ...
  kw = function() {
    return $((function(_this) {
      return function(event, send) {
        var description, glyph, glyphs, i, len, meta, name, raw_text, rsg, tag, text, type, uchr;
        if (!select(event, '.', Σ_glyph_description)) {
          return send(event);
        }
        type = event[0], name = event[1], description = event[2], meta = event[3];
