(function() {
      ...
      return function(event, send) {
        ...
        if (select(event, '!', 'columns')) {
          ...
          if (parameters.length === 0) {
            parameters.push(S.COLUMNS.count);
          }
  ...
  this._initialize_state = function(S) {
    var base_setting;
    if (S.COLUMNS != null) {
      throw new Error("namespace collision: `S.COLUMNS` already defined");
    }
    S.COLUMNS = {};
  ...
  this._push = function(S, setting) {
    return S.COLUMNS.stack.push(setting);
  };

  this._pop = function(S) {
    return S.COLUMNS.stack.pop();
  };

  this._get_column_count = function(S) {
    return S.COLUMNS.stack[this._get_stack_idx(S)]['count'];
  };

  this._get_stack_idx = function(S) {
    return S.COLUMNS.stack.length - 1;
  };
  ...
  this._restore_column_count = function(S, event, send) {
    ...
    this._start_column_region(S, event, send, column_count);
    return debug('©25119-3', S.COLUMNS.stack);
  };
  ...
  this._stop_column_region = function(S, event, send) {
    ...
    var column_count, last_state, meta;
    if ((this._get_stack_idx(S)) === 0) {
      return;
    }
    column_count = this._get_column_count(S);
    last_state = this._pop(S);
