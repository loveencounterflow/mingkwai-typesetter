      ...
      return function(event, send) {
        ...
        if (select(event, '!', 'slash')) {
          ...
          if (CND.isa_list(parameters)) {
            ...
            above = parameters.above, mid = parameters.mid, below = parameters.below;
            if (mid != null) {
    ...
    return $((function(_this) {
      return function(event, send) {
        ...
        if (select(event, '(', 'columns')) {
          ...
          type = event[0], name = event[1], _ = event[2], meta = event[3];
          event = ['!', name, ['pop'], meta];
        }
        if (select(event, '!', 'columns')) {
          type = event[0], name = event[1], parameters = event[2], meta = event[3];
      ...
      return function(event, send) {
        ...
        if (select(event, '(', 'multi-columns')) {
          ...
          if (column_count > 1) {
             ...
             */
            send(stamp([
              '.', 'COLUMNS/tex', "\\mktsVspace{1}\\begin{multicols}{" + column_count + "}\\raggedcolumns{}", copy(meta, {
                tex: 'pass-through'
              })
            ]));
          }
