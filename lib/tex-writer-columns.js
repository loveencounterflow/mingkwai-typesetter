(function() {
  ...
  this.$main = function(S) {
    return D.TEE.from_pipeline([this.$initialize_state(S), this.$end_columns_with_document(S), this.$slash(S), this.$columns(S), this.$transform_to_pretex(S)]);
  };
    ...
    return $((function(_this) {
      return function(event, send) {
        ...
        if (select(event, '(', 'multi-columns')) {
          ...
          if (column_count > 1) {
            send(stamp([
              '(', 'COLUMNS/group', null, copy(meta, {
                tex: 'pass-through'
              })
            ]));
            send(stamp([
              '.', 'COLUMNS/tex', "\\vspace{\\parskip}%TEX-WRITER/COLUMNS/$transform-to-tex\n", copy(meta, {
                tex: 'pass-through'
              })
            ]));
            send(stamp([
              '.', 'COLUMNS/tex', "\\begin{multicols}{" + column_count + "}\\raggedcolumns{}", copy(meta, {
                tex: 'pass-through'
              })
            ]));
          }
    ...
    return $((function(_this) {
      return function(event, send) {
        ...
        if (select(event, '(', 'COLUMNS/group')) {
          help('975', (JSON.stringify(event)).slice(0, 51));
          within_group = true;
        } else if (select(event, ')', 'COLUMNS/group')) {
          warn('975', (JSON.stringify(event)).slice(0, 51));
          if (all_whitespace) {
          ...
          urge('975', (JSON.stringify(event)).slice(0, 51));
          buffer.push(text);
        } else {
          if (within_group) {
            ...
            info('975', (JSON.stringify(event)).slice(0, 51));
            send(event);
          }
        }
        return null;
