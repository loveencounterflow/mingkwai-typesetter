(function() {
  ...
  this._push = function(S, setting) {
    return (S.sandbox.get('COLUMNS')).stack.push(setting);
  };

  this._pop = function(S) {
    return (S.sandbox.get('COLUMNS')).stack.pop();
  };

  this._get_column_count = function(S) {
    return (S.sandbox.get('COLUMNS')).stack[this._get_stack_idx(S)]['count'];
  };

  this._get_stack_idx = function(S) {
    return (S.sandbox.get('COLUMNS')).stack.length - 1;
  };
