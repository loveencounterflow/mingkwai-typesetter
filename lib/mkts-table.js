...
(function() {
  ...
  this._new_description = function(S) {
    ...
    R = {
      ...
      cellquads: {},
      /* cell extents in terms of quads, by cell designations */quadcells: {},
      /* which quads belong to what cells, by quad designations */quad_dimensions: {},
      cellborders: {},
  ...
  this.cellquads = function(me, text) {
    var base, cellquad, d, designation, name, ref1, type;
    if ((type = CND.type_of(text)) !== 'text') {
    ...
    ref1 = this._walk_cellquads(me, d);
    for (cellquad of ref1) {
      ((base = me.quadcells)[name = cellquad.designation] != null ? base[name] : base[name] = []).push(designation);
    }
    //.........................................................................................................
    return null;
  };
  ...
  this._parse_cellborder = function(me, cellborder) {
    var _, cellhint, cellhints, cells, d, groups, i, len, quadkeys, ref1, side, sides, style, type;
    ...
    cellhints = new Set((function() {
      var i, len, ref1, results;
      ref1 = cellhints.split(',');
      results = [];
      for (i = 0, len = ref1.length; i < len; i++) {
        _ = ref1[i];
        results.push(_.trim());
    ...
    if (cellhints.has('*')) {
      ...
      if (cellhints.has('table')) {
        ...
        for (i = 0, len = sides.length; i < len; i++) {
          side = sides[i];
          ref1 = this._walk_table_edge_quads(me, side);
          for (d of ref1) {
            cellhints.add(d.quadkey);
          }
        }
      }
      quadkeys = (function() {
        var results;
        results = [];
        for (cellhint of cellhints) {
          results.push(cellhint);
        }
        return results;
      })();
      cells = this._cellnames_from_quadkeys(me, quadkeys);
    }
    //.........................................................................................................
    style = style.trim();
    if (style === 'none' || style === '') {
  ...
  this._walk_debug_joints_events = function*(me) {
    var col_letter, col_nr, quadkey, ref1, ref2, row_nr, x, y, z;
    if (!me.debug) {
      return;
    }
  ...
  this._compute_cell_dimensions = function(me) {
    var bottom, cellquads, designation, height, left, ref1, right, top, width;
    ref1 = me.cellquads;
    ...
    for (designation in ref1) {
      ...
      width = right - left;
      height = bottom - top;
      me.cell_dimensions[designation] = {left, right, width, top, bottom, height};
    }
    return null;
  };
  ...
  //-----------------------------------------------------------------------------------------------------------
  this._cellnames_from_quadkeys = function(me, quadkeys) {
    var R, cellname, i, j, len, len1, quadcells, quadkey;
    R = new Set();
    for (i = 0, len = quadkeys.length; i < len; i++) {
      quadkey = quadkeys[i];
      if ((quadcells = me.quadcells[quadkey]) == null) {
        continue;
      }
      for (j = 0, len1 = quadcells.length; j < len1; j++) {
        cellname = quadcells[j];
        R.add(cellname);
      }
    }
    return (function() {
  ...
  //-----------------------------------------------------------------------------------------------------------
  this._walk_cellquads = function*(me, cellquads) {
    /* TAINT don't use EXCJSCC directly */
    var col_letter, col_nr, designation, i, j, ref1, ref2, ref3, ref4, row_nr;
    for (row_nr = i = ref1 = cellquads.top, ref2 = cellquads.bottom; (ref1 <= ref2 ? i <= ref2 : i >= ref2); row_nr = ref1 <= ref2 ? ++i : --i) {
      for (col_nr = j = ref3 = cellquads.left, ref4 = cellquads.right; (ref3 <= ref4 ? j <= ref4 : j >= ref4); col_nr = ref3 <= ref4 ? ++j : --j) {
        col_letter = (EXCJSCC.n2l(col_nr)).toLowerCase();
        designation = `${col_letter}${row_nr}`;
  ...
  //-----------------------------------------------------------------------------------------------------------
  this._walk_table_edge_cellnames = function*(me, edge) {
    var cellname, cellnames, d, i, len, ref1, seen_cellnames;
    seen_cellnames = new Set();
    ref1 = this._walk_table_edge_quads(me, edge);
    for (d of ref1) {
      if ((cellnames = me.quadcells[d.quadkey]) == null) {
        continue;
      }
      for (i = 0, len = cellnames.length; i < len; i++) {
        cellname = cellnames[i];
        if (seen_cellnames.has(cellname)) {
          continue;
        }
  ...
  //-----------------------------------------------------------------------------------------------------------
  this._walk_table_edge_quads = function*(me, edge) {
    /* TAINT don't use EXCJSCC directly */
    var col_letter, col_nr, col_nr_1, col_nr_2, i, j, quadkey, ref1, ref2, ref3, ref4, row_nr, row_nr_1, row_nr_2;
    switch (edge) {
      case 'left':
        col_nr_1 = 1;
        col_nr_2 = 1;
      ...
      case 'bottom':
        ...
        break;
      case '*':
        yield* this._walk_table_edge_quads(me, 'left');
        yield* this._walk_table_edge_quads(me, 'right');
        yield* this._walk_table_edge_quads(me, 'top');
        yield* this._walk_table_edge_quads(me, 'bottom');
        return;
      default:
