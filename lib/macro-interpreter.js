(function() {
  ...
  this.$process_actions = (function(_this) {
    return function(S) {
      ...
      var CS, VM, copy, hide, is_first, local_filename, macro_output, sandbox_backup, stamp;
      ...
      return $(function(event, send) {
        var _, action_value, action_value_rpr, changeset, error, error1, error2, error_message, js_source, language, line_nr, macro_output_rpr, meta, mode, raw_source, ref, ref1, wrapped_source;
        if (is_first) {
          is_first = false;
        ...
        if (MKTS.MD_READER.select(event, '.', 'action')) {
          ...
          if (error_message != null) {
            ...
            return sandbox_backup = MKTS.DIFFPATCH.patch(changeset, sandbox_backup);
          }
        } else {
          return send(event);
        }
