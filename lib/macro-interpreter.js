...
(function() {
  ...
  this.track_changes = function() {
    ...
    return (function(_this) {
      return function() {
        return _this._sandboxes[idx];
      };
    })(this);
  };

  this.$capture_change_events = (function(_this) {
    return function(S) {
      ...
      return $(function(event, send) {
        ...
        if (select(event, '~', 'change')) {
          ...
          ref = _this._sandboxes;
          for (idx = i = 0, len = ref.length; i < len; idx = ++i) {
            sandbox = ref[idx];
            _this._sandboxes[idx] = MK.TS.DIFFPATCH.patch(changeset, sandbox);
          }
          send(event);
        } else {
          send(event);
  ...
  this.$process_actions = (function(_this) {
    return function(S) {
      ...
      hide = MKTS.MD_READER.hide.bind(MKTS.MD_READER);
      select = MKTS.MD_READER.select.bind(MKTS.MD_READER);
      get_sandbox = _this.track_changes();
      CS = require('coffee-script');
      VM = require('vm');
      local_filename = 'XXXXXXXXXXXXX';
      macro_output = [];
      return $(function(event, send) {
        var _, action_value, action_value_rpr, changeset, error, error1, error2, error_message, js_source, language, line_nr, macro_output_rpr, meta, mode, raw_source, ref, ref1, sandbox, sandbox_backup, wrapped_source;
  ...
  this.$consolidate_regions = (function(_this) {
    return function(S) {
      ...
      return $(function(event, send) {
        ...
        /* TAINT code duplication */
        var _, call_signature, error_message, expected, extra, identifier, line_nr, message, meta, parameters, parameters_txt, ref, ref1;
        if (select(event, '(')) {
          _ = event[0], call_signature = event[1], extra = event[2], meta = event[3];
          ref = call_signature.match(/^\s*([^\s]*)\s*(.*)$/), _ = ref[0], identifier = ref[1], parameters_txt = ref[2];
  ...
  this._parameters_from_text = (function(_this) {
    return function(S, line_nr, text) {
      var CS, R, VM, error, error1, error2, error_message, js_source, ref, ref1, sandbox, source;
      if (/^\s*$/.test(text)) {
        return [null, []];
      }
