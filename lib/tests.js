(function() {
  ...
  this["MKTS._ESC.escape_region_macros"] = function(T, done) {
    var S, i, len, probe, probes_and_matchers, ref, registry_matcher, text_matcher, text_result;
    probes_and_matchers = [["some text here <<(em>>and some there<<em)>>", null], ["some text here \\<<(em>>and some there<<em)>>", null], ["some text here <<(em>>and some there\\<<em)>>", null], ["some text here <<(em>>and some there<<)>>", null], ["some text here \\<<(em>>and some there<<)>>", null], ["some text here <<(em>>and some there\\<<)>>", null], ["some text here <<(em>>and some there<<foo)>>", null]];
    for (i = 0, len = probes_and_matchers.length; i < len; i++) {
      ref = probes_and_matchers[i], probe = ref[0], text_matcher = ref[1], registry_matcher = ref[2];
      S = MKTS._ESC.initialize({});
