(function() {
  ...
  this.$main = (function(_this) {
    return function(S) {
      ...
      return D.TEE.from_pipeline([_this.$fontlist(S), _this.$most_frequent.with_fncrs.$rewrite_events(S), _this.$dump_db(S), _this.$most_frequent.$read(S), _this.$most_frequent.$assemble(S), _this.$most_frequent.$details_from_glyphs(S), _this.$most_frequent.with_fncrs.$format(S), _this.$most_frequent.with_fncrs.$assemble(S), _this.$dump_db.$format(S)]);
    };
  })(this);
  ...
  this.$dump_db.$format = (function(_this) {
    return function(S) {
      ...
      return $(function(event, send) {
        var _, details, meta, within_dumpdb;
        ...
        if (within_dumpdb && select(event, '.', 'details')) {
          ...
          send(['tex', " & ", copy(meta)]);
          send([
            '.', 'text', "x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x ", copy(meta, {
              'typofix': 'escape-ncrs'
            })
          ]);
          send(['tex', "\\\\\n", copy(meta)]);
          ...
          send(['tex', " & ", copy(meta)]);
          send([
            '.', 'text', "x x x x x x x x x x x x x x x x ", copy(meta, {
              'typofix': 'escape-ncrs'
            })
          ]);
          send(['tex', "\\\\\n", copy(meta)]);
