(function() {
  ...
  this.$expand = function(S) {
    ...
    pipeline = [this.$expand_command_and_value_macros(S), this.$expand_region_macros(S), this.$expand_action_macros(S), this.$expand_raw_macros(S), this.$expand_html_comments(S), this.$expand_escape_chrs(S), this.$expand_escape_illegals(S)];
    settings = {
      S: S
    };
