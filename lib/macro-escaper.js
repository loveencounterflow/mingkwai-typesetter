(function() {
  ...
  this.$expand = function(S) {
    ...
    pipeline = [this.$expand.$command_and_value_macros(S), this.$expand.$region_macros(S), this.$expand.$action_macros(S), this.$expand.$raw_macros(S), this.$expand.$html_comments(S), this.$expand.$escape_chrs(S)];
    settings = {
      S: S
    };
    ...
    return function(S) {
      return $(function(event, send) {
        ...
        if (MKTS.MD_READER.select(event, '.', 'text')) {
          ...
          debug('83457', rpr(_this.escape.remove_escaping_backslashes(S, text)));
