...
(function() {
  //###########################################################################################################
  var $, CND, D, HELPERS, MKNCR, MKTS, Markdown_parser, alert, assign, badge, debug, echo, get_parse_html_methods, help, info, jr, log, misfit, new_md_inline_plugin, njs_fs, njs_path, parse_methods, rpr, tracker_pattern, urge, warn, whisper,
    indexOf = [].indexOf;
  ...
  get_parse_html_methods = function() {
    ...
    // ◯ parse5                     2.1.5   ❯  5.1.0  https://github.com/inikulin/parse5
  ...
  this.create_md_read_tee = function(S, md_source) {
    ...
    readstream.pipe(this._PRE.$flatten_inline_tokens(S)).pipe(this._PRE.$flatten_image_tokens(S)).pipe(this._PRE.$reinject_html_blocks(S)).pipe(this._PRE.$rewrite_markdownit_tokens(S)).pipe(D.$observe((event) => {
      return debug('©1', jr(event));
