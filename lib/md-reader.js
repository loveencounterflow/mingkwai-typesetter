(function() {
  ...
  this._new_markdown_parser = function() {
    ...
    R.enable('text').enable('escape').enable('backticks').enable('strikethrough').enable('emphasis').enable('link').enable('image').enable('autolink').enable('html_inline').enable('entity').enable('fence').enable('blockquote').enable('hr').enable('list').enable('reference').enable('heading').enable('html_block').enable('table').enable('paragraph').enable('normalize').enable('block').enable('inline').enable('smartquotes');
  ...
  this._PRE.$extra_hr = (function(_this) {
    return function(S) {
      var pattern, within_code;
      pattern = /^(°{4,}|\.{4,}|:{4,}|-{4,}|={4,}|\^{4,}|v{4,})$/gm;
      within_code = false;
      return $(function(event, send) {
        var i, is_plain, len, meta, name, ref, results, stretch, text, type;
        if (_this.select(event, '(', ['code', 'code-span'])) {
          ...
          within_code = false;
          return send(event);
        } else if ((!within_code) && _this.select(event, '.', 'text')) {
          type = event[0], name = event[1], text = event[2], meta = event[3];
          is_plain = false;
          ...
          for (i = 0, len = ref.length; i < len; i++) {
            ...
            if (is_plain = !is_plain) {
              results.push(send(['.', 'text', stretch, _this.copy(meta)]));
            } else {
              results.push(send(['.', 'hr', stretch, _this.copy(meta)]));
            }
          }
          return results;
        } else {
