(function() {
  var $, CND, D, HELPERS, MKTS, Markdown_parser, alert, badge, debug, echo, get_parse_html_methods, help, info, log, misfit, new_md_inline_plugin, njs_fs, njs_path, parse_methods, rpr, tracker_pattern, urge, warn, whisper,
  ...
  this._new_markdown_parser = function() {
    ...
    R.enable('text').enable('escape').enable('backticks').enable('strikethrough').enable('emphasis').enable('link').enable('image').enable('autolink').enable('html_inline').enable('entity').enable('fence').enable('blockquote').enable('list').enable('reference').enable('heading').enable('html_block').enable('table').enable('paragraph').enable('normalize').enable('block').enable('inline').enable('smartquotes');
    R.use(require('markdown-it-footnote'));
    return R;
  };

  get_parse_html_methods = function() {
    var PARSE5, R, get_message;
  ...
  this._PRE.$flatten_inline_tokens = (function(_this) {
    return function(S) {
  ...
  this._PRE.$flatten_image_tokens = (function(_this) {
    return function(S) {
      return $(function(token, send) {
        var attribute_name, attribute_value, i, j, len, len1, ref, ref1, ref2, src, sub_token, type;
        if ((type = token['type']) === 'image') {
          src = null;
          ref = token['attrs'];
          for (i = 0, len = ref.length; i < len; i++) {
            ref1 = ref[i], attribute_name = ref1[0], attribute_value = ref1[1];
  ...
  this._PRE.$rewrite_markdownit_tokens = (function(_this) {
    return function(S) {
      ...
      return $(function(token, send) {
        var attribute_name, attribute_value, col_nr, extra, h_level, i, id, j, language_name, len, len1, line_nr, map, markup, meta, name, position, ref, ref1, ref2, ref3, ref4, ref5, ref6, ref7, tag, text_meta, type;
        _send = send;
        if (token === end_token) {
          ...
          if (within_footnote_block || !S.has_ended) {
            switch (type) {
              case 'paragraph_open':
                send(_this.hide(['~', 'start-paragraph', null, meta]));
                break;
              ...
              case 'link_open':
                ...
                for (i = 0, len = ref2.length; i < len; i++) {
                  ref3 = ref2[i], attribute_name = ref3[0], attribute_value = ref3[1];
              ...
              case 'tr_close':
                if (type === 'th_open' && (token['attrs'] != null)) {
                  ...
                  for (j = 0, len1 = ref4.length; j < len1; j++) {
                    ref5 = ref4[j], attribute_name = ref5[0], attribute_value = ref5[1];
                ...
                ref6 = type.split('_'), tag = ref6[0], position = ref6[1];
              ...
              case 'fence':
                ...
                break;
              case 'html_inline':
                ref7 = _this._parse_html_tag(token['content']), position = ref7[0], name = ref7[1], extra = ref7[2];
                switch (position) {
                  case 'comment':
  ...
  this._PRE.$issue_administrative_events = (function(_this) {
    return function(S) {
      return $(function(event, send) {
        var meta, name, text, type;
        type = event[0], name = event[1], text = event[2], meta = event[3];
        if (_this.select(event, '(', 'document')) {
          send(['~', 'start', null, _this.copy(meta)]);
          send(['~', 'flush', null, _this.copy(meta)]);
          send(event);
        } else if (_this.select(event, ')', 'document')) {
          send(event);
          send(['~', 'stop', null, _this.copy(meta)]);
        } else {
          send(event);
        }
        return null;
  ...
  this._PRE.$consolidate_tables = (function(_this) {
    return function(S) {
      ...
      description = null;
      return $(function(event, send) {
        var i, len, meta, name, past_event, ref, ref1, ref2, style, style_name, style_value, table_meta, text, type, within_table;
        type = event[0], name = event[1], text = event[2], meta = event[3];
        within_table = track.within('(table)');
        track(event);
        if (_this.select(event, '(', 'table')) {
          if (collecting) {
            return send(['.', 'warning', "detected nested tables", _this.copy(meta)]);
          }
          collecting = true;
          alignments = [];
          description = {};
          ...
          if (_this.select(event, '.', 'footnote-ref')) {
            ...
            if (within_footnote_def) {
              ...
              if (!target_idx) {
                ...
                send(['.', 'warning', message, _this.copy(meta != null ? meta : {})]);
              } else {
  ...
  this._PRE.$hr2 = (function(_this) {
    return function(S) {
      ...
      * asterisks (line)
      ...
      return $(function(event, send) {
        var _, above, below, match, meta, name, one, ref, ref1, ref2, ref3, slash, start, stop, text, three, two, type;
        send_ = send;
        if (_this.select(event, '(', ['code', 'code-span'])) {
          flush();
          code_count += +1;
        } else if (_this.select(event, ')', ['code', 'code-span'])) {
          code_count += -1;
        }
        if (code_count < 1 && _this.select(event, '~', 'start-paragraph', true)) {
          if (buffer != null) {
            throw new Error('XXXX');
          }
          buffer = [];
          return buffer.push(event);
        } else if ((buffer != null) && _this.select(event, '.', 'p')) {
          buffer.push(event);
          if (!((buffer.length === 3) && _this.select(buffer[1], '.', 'text'))) {
            return flush();
          }
          ref = buffer[1], type = ref[0], name = ref[1], text = ref[2], meta = ref[3];
          match = text.match(analyzer);
          if (match == null) {
            return flush();
          }
          buffer = null;
          _ = match[0], start = match[1], above = match[2], one = match[3], two = match[4], three = match[5], below = match[6], stop = match[7];
          if (start !== stop) {
          ...
          one = (ref1 = one[0]) != null ? ref1 : '';
          two = (ref2 = two[0]) != null ? ref2 : '';
          three = (ref3 = three[0]) != null ? ref3 : '';
          return send([
            '.', 'hr2', {
              slash: slash,
              above: above,
  ...
  this.select = function(event, type, name, hidden) {
    ...
    return (function(_this) {
      return function(md_source) {
        ...
        if (CND.isa_text(md_source)) {
          ...
          md_source = _this.TYPOFIX.rewrite(S, md_source);
          environment = {};
...
//# sourceMappingURL=../sourcemaps/md-reader.js.map
