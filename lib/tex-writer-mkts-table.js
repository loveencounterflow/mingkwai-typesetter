...
(function() {
  ...
  //-----------------------------------------------------------------------------------------------------------
  this.copy_layout = function(S, L, old_layout_name, new_layout_name) {
    var new_layout, old_layout;
    if (L.layouts[new_layout_name] != null) {
      throw new Error(`${badge} µ36339 refusing to re-define layout ${rpr(new_layout_name)}`);
    }
    old_layout = this.layout_from_name(S, L, old_layout_name);
    new_layout = CND.deep_copy(old_layout);
    new_layout.name = new_layout_name;
    this.store_layout(S, L, new_layout);
    this._initialize_layout(S, L, new_layout_name);
    return null;
  };
